...
class AudioLevelDetector:
    ...
    def __init__(self, num_keywords=1):
        ...
        logger.warning("AudioLevelDetector initialized as Porcupine fallback")
        logger.warning("WARNING: Using audio level detection instead of wake words!")
        logger.warning("You will need to speak VERY LOUDLY to trigger detection")
    ...
    def process(self, pcm):
        ...
        if len(pcm) > 0:
            ...
            if rms > self.threshold:
                logger.info(f"AudioLevelDetector LOUD SOUND DETECTED! RMS: {rms:.1f}")
...
def create(keyword_paths=None, sensitivities=None, model_path=None, library_path=None):
    ...
    try:
        ...
        try:
            ...
            if access_key:
                logger.info("Found Porcupine access key in environment")
                try:
                    ...
                    logger.info("Successfully created real Porcupine instance!")
                    return porcupine
                except Exception as e:
                    logger.error(f"Error creating Porcupine with access key: {e}")
            else:
                # Try to access the C library directly through a hack
                # This might work on some systems where the library is already loaded
                logger.info("No access key found, trying alternative approach...")
                ...
                try:
                    ...
                    if hasattr(pvporcupine, '_util'):
                        ...
                        logger.info("Patched library validator")
                    ...
                    try:
                        ...
                        logger.info("Successfully created real Porcupine instance without key!")
                        return porcupine
                    except Exception as e:
                        logger.error(f"Error creating Porcupine without key: {e}")
                        raise
            ...
            logger.error(f"Error setting up Porcupine: {e}")
            
    except ImportError:
        logger.warning("pvporcupine not installed")
    
    # If we reached here, we need to use the fallback
    logger.warning("Using audio level detection fallback for wake word detection")
