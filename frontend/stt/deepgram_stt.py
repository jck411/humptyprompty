...
class DeepgramSTT(QObject):
        ...
        try:
            ...
            if self.dg_connection:
                try:
                    ...
                    logging.debug("Deepgram connection finish cancelled.")
                except Exception as e:
            ...
            logging.debug("STT stop cancelled")
            if self.microphone:
                self.microphone.finish()
                self.microphone = None
            if self.dg_connection:
    ...
    def _activate_keepalive(self):
        if self.keepalive_active:
            return
        logging.debug("Activating KeepAlive mode")
        if self.microphone:
            self.microphone.finish()
            self.microphone = None
    ...
    async def _send_keepalive_messages(self):
        try:
            interval = 5
            logging.debug(f"KeepAlive loop every {interval}s")
            ...
            logging.debug("KeepAlive loop cancelled")
        except Exception as e:
    ...
    def _deactivate_keepalive(self):
        if not self.keepalive_active:
            return
        logging.debug("Deactivating KeepAlive mode")
        if self._keepalive_task and not self._keepalive_task.done():
            self._keepalive_task.cancel()
            self._keepalive_task = None
        if not self.microphone and self.dg_connection:
    ...
    async def stop_async(self):
        ...
        self.is_paused = False
        self.enabled_changed.emit(False)
        logging.debug("STT fully stopped (async)")
    ...
    async def shutdown(self, signal, loop):
        logging.debug(f"Shutdown signal received: {signal}")
        if self.microphone:
            self.microphone.finish()
        if self.dg_connection:
            await self.dg_connection.finish()
